...
\begin{itemize}
    ...
    \item \textbf{Software \& Toolchain:} \textbf{uv} (package \& project management), \textbf{Docker} (containerization), Git, GitHub Actions (CI/CD), SQLModel, Parquet, and LaTeX.
    ...
    \item \textbf{AI Tools:} \textbf{Claude Code}, Google Gemini, Gemini CLI, ChatGPT, and Google Antigravity for automated software engineering, academic and scientific research, and coding.
\end{itemize}
...
\begin{itemize}
    \item \versiontext{Engineered a modular, platform-agnostic trading system for weather-derivative prediction markets (e.g., Polymarket) using an asynchronous event-bus architecture in Python. Implemented a watcher-based service for real-time polling of weather providers and high-frequency market state through WebSockets and local orderbook monitoring.}{Developed an event-driven engine for capturing alpha in weather-derivative prediction markets. Features a provider-agnostic collection layer, asynchronous watcher architecture for real-time signal generation, and automated execution with velocity rotation, local orderbook tracking, and risk-aware position sizing via the Kelly Criterion.}
    \item \industryversion{Architected a multi-repository ecosystem with specialized submodules for data ingestion, core logic, and strategy execution, ensuring scalable infrastructure with integrated risk management and automated safety controls.}
    \item Skills: Python, Asyncio, WebSockets, Parquet, SQLModel, Risk Management, Docker.
\end{itemize}
