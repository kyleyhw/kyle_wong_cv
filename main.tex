...
International Baccalaureate (IB) Diploma in 6 subjects (Higher Level: Physics, Mathematics, Computer Science)}
}
...
\begin{itemize}
    ...
    \industryversion{\item \textbf{Financial Experience:} Manual trading in prediction markets (Polymarket) and stocks.}
...
\end{itemize}}
\industryversion{\textbf{agent\_evolve} (\url{https://github.com/kyleyhw/agent-evolve}) (work in progress)
\begin{itemize}
    \item A multi-model, cooperative evolutionary code-optimisation agent skill. Supervisor, explorer, and reviewer roles can each be filled by any LLM through CLI, with candidates evolving on isolated git branches. Includes interactive visualization and surfaces the winner as a pull request for human approval.
\end{itemize}}
}
