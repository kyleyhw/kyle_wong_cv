...
\textbf{mercury} [Private Repository]
\begin{itemize}
    \item \versiontext{Engineered a modular, platform-agnostic trading system for weather-derivative prediction markets (e.g., Polymarket) using an asynchronous event-bus architecture in Python. Implemented a watcher-based service for real-time polling of weather providers and high-frequency market state through WebSockets and local orderbook monitoring; deployed dockerized on AWS for 24/7 automated operation.}{Developed an event-driven engine for capturing alpha in weather-derivative prediction markets. Features a provider-agnostic collection layer, asynchronous watcher architecture for real-time signal generation, and automated execution with velocity rotation, local orderbook tracking, and risk-aware position sizing via the Kelly Criterion.}
    \item \industryversion{Architected a multi-repository ecosystem with specialized submodules for data ingestion, core logic, and strategy execution, managing dockerized infrastructure via AWS cloud-hosting with integrated risk management and automated safety controls.}
...
\begin{itemize}
    \item \versiontext{An application of constraint programming to the vehicle routing problem, assigning passengers to drivers by minimizing a Haversine distance-based cost function. Developed to provide a scalable framework for real-world logistical challenges like ride-sharing coordination.}{An optimization tool to solve the problem of assigning passengers to drivers for group trips. The tool uses an algorithm to find an optimal assignment based on driver and passenger constraints.}
